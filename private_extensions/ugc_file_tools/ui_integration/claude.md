...
- **头部空间收敛**：导入/导出中心不再在顶部占用大段说明文本；界面空间优先留给左侧资源选择与右侧配置/表格。
...
- **资源与选择口径**：`graph_selection.py` 统一“选图模型”参数形态；`resource_picker.py` 提供 project/shared 资源树选择器（元件/实体摆放/玩家模板 JSON 的叶子节点优先展示 JSON 内的 `name`，仅扫描文件头以避免大文件卡 UI；并过滤 `templates_index.json`/`instances_index.json`/`player_templates_index.json` 与 `原始解析/*.pyugc.json` 等辅助产物，避免误选进入导出/写回流程）。
  ...
  - 导出中心 UI 中 `mgmt_cfg` 分类标题显示为“管理配置（信号/结构体）”；关卡实体自定义变量不在资源树中勾选，需在 GIL 页右侧“关卡实体自定义变量（可选）”选择（左侧摘要与执行预览会显示已选数量）。
...
- **导出中心（关卡实体自定义变量）**：GIL 模式在左侧资源树提供“关卡实体自定义变量（全部）”勾选项；启用后会自动收集并全量补齐写入输出 `.gil` 的关卡实体变量（优先按关卡实体实例 `metadata.custom_variable_file` 引用到的变量文件过滤；缺失时回退到 `auto_custom_vars__level__<package_id>`）；候选/写回均以 payload 顶层 `owner` 做 fail-fast 约束（仅允许 `owner=="level"`），避免把玩家/第三方变量混入关卡实体写回。
