...
- **统一入口**：PACKAGES 页工具栏按钮（`package_toolbar.py`）→ `export_wizard.py`（非模态向导式对话框，导入/导出入口统一在此，并提供“分析中心”入口）。
- **分析中心**：`analysis_center/` 提供三步式分析对话框（范围选择/搜索与结果/构建索引），用于按节点/复合节点/信号名查询“在哪些节点图中使用”，并在执行页展示进度与失败清单。
- **中心对话框骨架**：`center_dialog_scaffold.py` 统一非模态中心对话框的创建（单例唤起、尺寸自适配、基础样式、标题栏、tabs 容器），供导入中心/分析中心复用，避免重复拷贝对话框样板代码。
...
- **子进程执行**：`_cli_subprocess.py` 封装运行 `ugc_file_tools` CLI（隐藏控制台窗口、解析进度、收集尾部日志）；在 Windows 下优先复用当前解释器（`python.exe/pythonw.exe`），仅在宿主程序 exe 场景下搜索可用的 `python.exe`；并合并捕获 stdout+stderr，避免错误信息落在 stdout 时被吞导致“stderr 为空”。
- **资源与选择口径**：`graph_selection.py` 统一“选图模型”参数形态；`resource_picker.py` 提供 project/shared 资源树选择器（元件/实体摆放/玩家模板 JSON 的叶子节点优先展示 JSON 内的 `name`，仅扫描文件头以避免大文件卡 UI；并过滤 `templates_index.json`/`instances_index.json`/`player_templates_index.json` 与 `原始解析/*.pyugc.json` 等辅助产物，避免误选进入导出/写回流程）。
  ...
  - 资源树在“程序化增删勾选（联动）”触发的内部重建时，会尽量保留当前展开节点与滚动位置，避免勾选一次就把视图重置到初始状态。
...
- **导出中心 wiring 收口**：`export_center_dialog_wiring/` 承载 `wire_export_center_dialog(...)` 的 wiring/联动实现；`export_center_dialog_controller.py` 保持为对外薄入口，避免单文件过大与耦合堆叠。
...
- **导出中心（GIA 回填）**：GIA 模式提供“基底 `.gil`”用于节点图 `entity_key/component_key` 回填；并支持可选“占位符参考 `.gil`”覆盖（留空=使用基底）。
  - 回填依赖清单扫描会同时覆盖 Graph Code 头部 `mount:` metadata（entity_key/component_key），避免“回填分析已就绪但写回挂载阶段缺映射”导致导出失败。
...
- **导出中心（GIL 冲突提示）**：GIL 写回前若检测到基础 .gil 中存在同名资源（UI布局/节点图/元件/实体），会弹出统一的“资源冲突检查”对话框供用户逐个决策（覆盖/新增/跳过）。界面内置了标准化的表格样式、下拉框样式与行列自适应展示以提升可读性。其中，UI 布局同名冲突弹窗会额外明确提示触发条件：仅在启用“UI 写回”时出现；若仅勾选“UI 回填记录”则不会触发写回与覆盖。
...
- **回填识别（UI bundle 预更新）**：当启用 UI 写回且用户勾选了若干 `UI源码/*.html` 页面时，识别会在启动子进程前按需生成/更新这些页面的 Workbench bundle（依赖 Playwright/Chromium），避免缺 bundle 导致识别失败。
- **步骤2（回填分析）双标签页**：回填表按“缺失/待修复”和“已就绪”拆分为两个标签页；标签标题显示条目计数，用于替代冗长的汇总文本。
- **回填识别表格可读性**：表格列宽采用“短列自适应、长列优先占宽”的策略（类别/值/状态自适应，依赖项/备注占剩余空间）；单元格 tooltip 保留完整文本，便于核对被省略的长 key。
- **回填识别表格可复制**：回填分析表格支持选中行按 Ctrl+C 复制为 TSV，便于粘贴到表格/聊天。
- **交互收敛**：步骤2不提供“清空结果”按钮；需要重新识别时直接再次点击“识别”即可。
- **进度条固定底部**：步骤2“识别”与步骤3“执行”的进度条位于各步骤页底部固定区域，不随内容滚动。
- **执行页失败复现信息**：当导出/写回子进程失败时，执行页“结果摘要”会汇总本次导出的上下文（当前项目、基地/基底输入、已选资源与全部可选资源快照、各配置项可选值与当前选择、plan/selection-json、CLI 模板）并提供“复制”按钮一键复制整段文本用于复现。
...
- **Legacy**：历史 `export_*.py` 导出入口保留为**兼容薄封装**（统一跳转 `export_wizard.py` 的导出中心，并可指定默认格式），不再维护独立导出对话框；`read_*.py` 已降级为**兼容薄封装**（默认跳转 `import_center/` 的导入中心，并预选对应任务类型）。
...
- 数值状态码（如 `exit_code`）不要用 `x or fallback` 兜底：成功值 `0` 会被当成 false，导致 UI 误判失败（典型症状：report 已生成但提示 “exit=1 / stderr 为空”）。
