...
def _clean_and_collect_instance_infos(
    *, plan_obj: Any, precheck_skipped_inputs: list[dict[str, str]]
) -> tuple[Any, list[dict[str, object]]]:
    """预检过滤实例 JSON 并收集用于冲突检查的实例信息。"""

    import json

    selected_infos: list[dict[str, object]] = []
    cleaned_instance_files: list[Path] = []
    for p in list(getattr(plan_obj, "selected_instance_json_files", []) or []):
        path = Path(p).resolve()
        if not path.is_file():
            record_precheck_skip(precheck_skipped_inputs=precheck_skipped_inputs, category="instances", file_path=path, reason="文件不存在")
            continue
        if path.name == "instances_index.json":
            record_precheck_skip(
                ...
                category="instances",
                file_path=path,
                reason="instances_index.json 是索引列表，不是单实例 JSON（每文件一个实例 dict）",
            )
            continue
        obj = json.loads(path.read_text(encoding="utf-8"))
        if not isinstance(obj, dict):
            record_precheck_skip(precheck_skipped_inputs=precheck_skipped_inputs, category="instances", file_path=path, reason="不是单实例 JSON（期望 dict）")
            continue
        instance_id_text = str(obj.get("instance_id") or "").strip()
        if instance_id_text == "":
            record_precheck_skip(precheck_skipped_inputs=precheck_skipped_inputs, category="instances", file_path=path, reason="缺少 instance_id")
            continue
        cleaned_instance_files.append(path)
        instance_name = str(obj.get("name") or "").strip() or instance_id_text
        selected_infos.append({"instance_json_file": path, "instance_id_text": instance_id_text, "instance_name": instance_name})

    if len(cleaned_instance_files) != len(list(getattr(plan_obj, "selected_instance_json_files", []) or [])):
        plan_obj = replace(plan_obj, selected_instance_json_files=list(cleaned_instance_files))
    return plan_obj, list(selected_infos)


def _build_conflict_instances(
    *, selected_infos: list[dict[str, object]], base_instance_id_by_name: dict[str, int], scan_ok: bool
) -> list[dict[str, object]]:
    """构造实例同名冲突条目列表供 UI 弹窗展示。"""
    ...
    return list(conflict_instances)


def _prompt_instance_conflicts_dialog(
    *, QtCore: Any, dialog: Any, conflict_instances: list[dict[str, object]]
    ...
    return open_export_center_gil_instance_conflicts_dialog(
        QtCore=QtCore,
        QtWidgets=QtWidgets,
        Colors=Colors,
        Sizes=Sizes,
        parent_dialog=dialog,
        conflict_instances=list(conflict_instances),
    )


def _apply_instance_conflict_choices(
    *,
    plan_obj: Any,
    selected_infos: list[dict[str, object]],
    conflict_instances: list[dict[str, object]],
    ...
    """将实例冲突用户选择写回到 plan（含 new_instance_name 分配与 skip 过滤）。"""
    ...
    by_file_cf: dict[str, dict[str, object]] = {str(Path(str(it["instance_json_file"])).resolve()).casefold(): dict(it) for it in selected_infos}
    new_name_by_inst_file_cf: dict[str, str] = {}
    for fp_cf, it in list(by_file_cf.items()):
        if actions_by_inst_file_cf.get(fp_cf) == "add":
            if bool(scan_ok):
                new_name_by_inst_file_cf[fp_cf] = alloc_new_name_sequential(base_name=str(it["instance_name"]), used_casefold=used_inst_names_cf)
            else:
                new_name_by_inst_file_cf[fp_cf] = alloc_new_name_fallback(base_name=str(it["instance_name"]))

    resolutions: list[dict[str, str]] = []
    for it in list(conflict_instances):
        ...
        if act2 == "add":
            resolutions.append(
                {"instance_json_file": rp, "action": "add", "new_instance_name": str(new_name_by_inst_file_cf.get(rp.casefold(), "") or "").strip()}
            )
        elif act2 == "skip":
            resolutions.append({"instance_json_file": rp, "action": "skip"})
        else:
            resolutions.append({"instance_json_file": rp, "action": "overwrite"})
    ...
    return replace(
        ...
        instance_conflict_resolutions=list(resolutions),
    )


def resolve_gil_instance_conflicts(
    *,
    QtCore: Any,
    dialog: Any,
    main_window: Any,
    ...
    """处理实体实例同名冲突策略并更新 plan。"""

    from ..plans import _ExportGilPlan

    if not isinstance(plan_obj, _ExportGilPlan) or not bool(plan_obj.selected_instance_json_files):
        return plan_obj

    plan_obj, selected_infos = _clean_and_collect_instance_infos(plan_obj=plan_obj, precheck_skipped_inputs=precheck_skipped_inputs)
    ...
    base_instance_id_by_name = extract_int_map(base_report=base_report, key="instance_id_by_name")
    conflict_instances = _build_conflict_instances(
        selected_infos=list(selected_infos),
        base_instance_id_by_name=dict(base_instance_id_by_name),
        scan_ok=bool(scan_ok),
    )
    if not conflict_instances:
        return plan_obj

    user_choices = _prompt_instance_conflicts_dialog(QtCore=QtCore, dialog=dialog, conflict_instances=list(conflict_instances))
    if user_choices is None:
        return None

    return _apply_instance_conflict_choices(
        plan_obj=plan_obj,
        selected_infos=list(selected_infos),
        conflict_instances=list(conflict_instances),
        base_instance_id_by_name=dict(base_instance_id_by_name),
...
__all__ = [
    "resolve_gil_instance_conflicts",
]
