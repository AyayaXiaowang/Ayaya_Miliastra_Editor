...
from ugc_file_tools.node_graph_semantics.graph_model import normalize_edges_list as _normalize_edges_list
from ugc_file_tools.node_graph_semantics.graph_model import normalize_nodes_list as _normalize_nodes_list
from ugc_file_tools.node_graph_semantics.layout import build_pos_transform as _build_pos_transform
...
from .pipeline_entry_ops import _try_find_existing_graph_entry_in_groups_list
from .pipeline_entry_ops import apply_overwrite_policy_and_append_if_needed_for_template_clone
...
from .pipeline_graph_variables_merge import _normalize_graph_variable_def_table_inplace
from .pipeline_graph_variables_merge import build_graph_variables_table_for_entry
from .pipeline_placeholders import _classify_component_key_placeholder_policy_for_writeback
from .pipeline_placeholders import _classify_entity_key_placeholder_policy_for_writeback
from .pipeline_placeholders import _reset_placeholder_registries_after_writeback
from .pipeline_placeholders import _resolve_component_id_registry_for_writeback
from .pipeline_placeholders import _resolve_entity_id_registry_for_writeback
from .pipeline_placeholders import _set_placeholder_registries_for_writeback
from .pipeline_port_types import _ensure_graph_model_enriched_port_types_inplace
from .pipeline_port_types import _repair_and_validate_dict_mutation_port_types_or_raise
from .pipeline_port_types import _write_port_type_gap_report_and_fail_if_any_or_raise
from .pipeline_signals import _extract_signal_node_def_id_maps_from_payload_root
from .pipeline_templates import _TemplateGraphSample
from .pipeline_ui_keys import _prepare_ui_key_to_guid_registry_for_writeback
from .pipeline_ui_keys import _validate_required_ui_keys_or_raise
from .pipeline_ui_registry_legacy import _try_load_ui_key_to_guid_registry_for_graph_model

from .pipeline_mode_template_clone import _allocate_graph_id as _allocate_graph_id_from_payload
from .pipeline_mode_template_clone import _build_new_entry_from_template as _build_new_entry_from_template_obj


_NODE_GRAPH_SECTION_KEY = "10"
_NODE_GRAPH_GROUPS_KEY = "1"
_NODE_GRAPH_SECTION_VERSION_KEY = "7"
_NODE_GRAPH_SECTION_VERSION_DEFAULT = 0
...
def _merge_ui_guid_registry_legacy_inplace(
    *, graph_model_json_path: Path, required_ui_keys: set[str], ui_key_to_guid_registry: Dict[str, int]
    ...
    """Merge legacy ui registry mapping into a ui_key_to_guid_registry dict."""
...
def _sort_nodes_or_raise(*, graph_model: Dict[str, Any]) -> List[Any]:
    """Sort graph nodes into a stable order for writeback."""
...
def _ensure_node_graph_section_from_template_if_missing_inplace(
    *, payload_root: Dict[str, Any], template_section: Dict[str, Any]
    ...
    """Ensure payload_root contains a node graph section using a template fallback."""
    section = payload_root.get(str(_NODE_GRAPH_SECTION_KEY))
    if isinstance(section, dict):
        return section
    import copy

    section = copy.deepcopy(template_section)
    section[str(_NODE_GRAPH_GROUPS_KEY)] = []
    section[str(_NODE_GRAPH_SECTION_VERSION_KEY)] = int(_NODE_GRAPH_SECTION_VERSION_DEFAULT)
    payload_root[str(_NODE_GRAPH_SECTION_KEY)] = section
    return section


def _normalize_groups_list_inplace(section10: Dict[str, Any]) -> List[Any]:
    """Normalize section10 groups list field to a list."""
    value = section10.get(str(_NODE_GRAPH_GROUPS_KEY))
    if isinstance(value, list):
        return value
    if isinstance(value, dict):
        section10[str(_NODE_GRAPH_GROUPS_KEY)] = [value]
        return section10[str(_NODE_GRAPH_GROUPS_KEY)]
    section10[str(_NODE_GRAPH_GROUPS_KEY)] = []
    return section10[str(_NODE_GRAPH_GROUPS_KEY)]
...
def _apply_constants_and_maybe_prune_assembly_list(
    *,
    ...
    """Apply constants/outparam types and then optionally prune assembly-list unused pins."""
...
def _write_edges_and_sort_pins_inplace(
    *,
    ...
    """Write edges into node pin records and sort pin records stably."""
...
def _prepare_graph_inputs(*, graph_model_json_path: Path, template_graph_id_int: int) -> _PreparedGraphInputs:
    """Load and normalize GraphModel JSON for template-clone writeback."""
    ...
    return _PreparedGraphInputs(
        graph_json_object=dict(inputs.graph_json_object),
        placeholders=inputs.placeholders,
        required_ui_keys=set(inputs.required_ui_keys),
        layout_name_hint=(str(inputs.layout_name_hint).strip() if inputs.layout_name_hint else None),
        graph_model=dict(inputs.graph_model),
...
@dataclass(frozen=True, slots=True)
class _PreparedRegistries:
    ui_key_to_guid_registry: Dict[str, int]
    ui_guid_registry_legacy_source: str | None
...
def _prepare_registries(
    *,
    ...
    """Prepare UI/component/entity registries used for placeholder writeback."""
    ...
    allow_unresolved_component_keys, missing_component_names = _classify_component_key_placeholder_policy_for_writeback(
        required_component_names=set(getattr(placeholders, "required_component_names", set()) or set()),
    ...
    allow_unresolved_entity_keys, missing_entity_names = _classify_entity_key_placeholder_policy_for_writeback(
        required_entity_names=set(getattr(placeholders, "required_entity_names", set()) or set()),
        entity_name_to_guid=entity_name_to_guid,
    )
    return _PreparedRegistries(
        ...
        missing_component_names=list(missing_component_names),
        entity_name_to_guid=dict(entity_name_to_guid),
        allow_unresolved_entity_keys=bool(allow_unresolved_entity_keys),
        missing_entity_names=list(missing_entity_names),
    )


def apply_template_clone_graph_writeback_inplace(
    *,
    graph_model_json_path: Path,
    template_sample: _TemplateGraphSample,
    ...
    """Apply one template-clone graph writeback into payload_root without writing output bytes."""
    prepared = _prepare_graph_inputs(graph_model_json_path=Path(graph_model_json_path), template_graph_id_int=int(template_graph_id_int))
    regs = _prepare_registries(
        graph_model_json_path=Path(graph_model_json_path),
        placeholders=prepared.placeholders,
        required_ui_keys=set(prepared.required_ui_keys),
        layout_name_hint=prepared.layout_name_hint,
        base_raw_dump_object=dict(base_raw_dump_object),
        preloaded_ui_key_to_guid_for_writeback=preloaded_ui_key_to_guid_for_writeback,
        preloaded_component_name_to_id=preloaded_component_name_to_id,
    ...
    graph_variables, graph_variables_ui_autofill_report = _maybe_autofill_graph_variables_from_ui_registry(
        graph_variables=list(prepared.graph_variables),
        ui_key_to_guid_registry=dict(regs.ui_key_to_guid_registry),
        enabled=bool(auto_fill_graph_variable_defaults_from_ui_registry),
        excluded_names=(set(ui_registry_autofill_excluded_graph_variable_names) if ui_registry_autofill_excluded_graph_variable_names else None),
        base_raw_dump_object=base_raw_dump_object,
    )
    ...
    section10 = _ensure_node_graph_section_from_template_if_missing_inplace(
        payload_root=payload_root,
        template_section=template_sample.template_section,
    )
    groups_list = _normalize_groups_list_inplace(section10)

    template_lib = build_template_library(
        template_nodes=template_sample.template_nodes,
        template_node_id_set=set(template_sample.template_node_id_set),
        template_library_dir=template_library_dir,
        effective_base_gil_path=Path(effective_base_gil_path),
    )
    name_to_type_id = _build_node_name_to_type_id(mapping_path=Path(mapping_path), scope=str(prepared.scope))
    ...
    active_package_id = prepared.graph_json_object.get("active_package_id")
    if not isinstance(active_package_id, str) or active_package_id.strip() == "":
        meta = prepared.graph_json_object.get("metadata")
    ...
    node_defs_by_name = _load_node_defs_by_scope(graph_generater_root=Path(graph_generater_root), scope=str(prepared.scope))
    node_defs_by_name.update(_load_node_defs_by_name_from_registry(workspace_root=Path(graph_generater_root), scope=str(prepared.scope)))

    composite_artifacts = _composite_writeback.build_composite_writeback_artifacts(
        graph_model=prepared.graph_model,
        graph_scope=str(prepared.scope),
        workspace_root=Path(graph_generater_root),
        graph_generater_root=Path(graph_generater_root),
        mapping_path=Path(mapping_path),
        node_defs_by_name=dict(node_defs_by_name),
        signal_maps=signal_maps,
        graph_variable_type_text_by_name=dict(prepared.graph_variable_type_text_by_name),
    ...
    sorted_nodes = _sort_nodes_or_raise(graph_model=prepared.graph_model)
    (
    ...
    edges = _normalize_edges_list(prepared.graph_model)
    ...
    transform_pos = _build_pos_transform(
        graph_json_object=prepared.graph_json_object,
    ...
    allow_unresolved_ui_keys, optional_hidden_missing_ui_keys = _validate_required_ui_keys_or_raise(
        graph_model_json_path=Path(graph_model_json_path),
        graph_json_object=dict(prepared.graph_json_object),
        required_ui_keys=set(prepared.required_ui_keys),
        ui_key_to_guid_registry=dict(regs.ui_key_to_guid_registry),
        base_gil_path=Path(effective_base_gil_path),
        base_raw_dump_object=base_raw_dump_object,
        layout_name_hint=(str(prepared.layout_name_hint).strip() if prepared.layout_name_hint else None),
    )

    _set_placeholder_registries_for_writeback(
        ui_key_to_guid_registry=dict(regs.ui_key_to_guid_registry),
        allow_unresolved_ui_keys=bool(allow_unresolved_ui_keys),
        component_name_to_id=dict(regs.component_name_to_id),
        allow_unresolved_component_keys=bool(regs.allow_unresolved_component_keys),
        entity_name_to_guid=dict(regs.entity_name_to_guid),
        allow_unresolved_entity_keys=bool(regs.allow_unresolved_entity_keys),
    )
    ...
    allocated_graph_id = int(
        _allocate_graph_id_from_payload(
            payload_root=payload_root,
    ...
    new_entry = _build_new_entry_from_template_obj(
        template_entry=template_sample.template_entry,
    ...
    return {
        ...
        "output_gil": str(Path(output_gil_path).resolve()),
        "template_graph_id_int": int(template_graph_id_int),
        ...
        **dict(edge_counts),
        ...
        "ui_custom_variable_sync": {"applied": False, "reason": "deferred_to_batch_finalize"},
        "ui_key_allow_unresolved_effective": bool(allow_unresolved_ui_keys),
        "ui_guid_registry_legacy_source": regs.ui_guid_registry_legacy_source,
        "optional_hidden_missing_ui_keys": list(optional_hidden_missing_ui_keys),
        "component_key_allow_unresolved_effective": bool(regs.allow_unresolved_component_keys),
        "component_key_missing_components": list(regs.missing_component_names),
        "entity_key_allow_unresolved_effective": bool(regs.allow_unresolved_entity_keys),
        "entity_key_missing_entities": list(regs.missing_entity_names),
        ...
        "enum_constants_skipped_report": None,
    }


__all__ = ["apply_template_clone_graph_writeback_inplace"]
