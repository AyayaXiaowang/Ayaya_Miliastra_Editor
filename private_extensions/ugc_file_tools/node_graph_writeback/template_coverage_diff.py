...
"""
Node template 覆盖差异报告（库层实现）。

说明：
- 原本逻辑存在于 `ugc_file_tools.commands.report_node_template_coverage_diff`（入口层）；
- 写回链路（node_graph_writeback）需要在写回前做预检，因此必须下沉到库层，避免反向依赖 commands/。
"""
...
def write_node_template_coverage_diff_report(
    *,
    graph_models: Sequence[Path],
    template_gil_path: Path,
    base_gil_path: Path,
    template_graph_id_int: int,
    template_library_dir: Path | None,
    mapping_path: Path,
    default_scope: str,
    output_json_name: str,
) -> Path:
    template_gil_path = Path(template_gil_path).resolve()
    if not template_gil_path.is_file():
        raise FileNotFoundError(str(template_gil_path))

    base_gil_path = Path(base_gil_path).resolve()
    ...
    default_scope = normalize_scope_or_default(str(default_scope), default_scope="server")
    name_to_ids_by_scope = _load_name_to_type_ids_by_scope(mapping_path)

    # ===== 1) 收集模板库已覆盖的 node type_id =====
    available_type_ids: Set[int] = set()
    available_type_ids |= collect_node_type_ids_from_gil_graph(
        gil_path=template_gil_path,
        graph_id_int=template_graph_id_int,
    )

    if template_library_dir is not None:
        template_library_dir = Path(template_library_dir).resolve()
    ...
    graph_json_files = iter_graph_model_json_files_from_paths([str(p) for p in list(graph_models or [])])
    ...
    for json_path in graph_json_files:
        ...
        for node in nodes:
            ...
            composite_id = str(node.get("composite_id") or "").strip()
            if composite_id == "" and node_def_kind == "composite" and isinstance(node_def_ref, dict):
                composite_id = str(node_def_ref.get("key") or "").strip()

            # 复合节点：type_id/模板覆盖不依赖 node_type_semantic_map 或模板库样本（写回会走复合节点专用工序 + schema 兜底）。
            # 因此预检阶段不将其计入“title→type_id 映射缺失 / 模板缺失”的阻断条件。
            if composite_id != "" or node_def_kind == "composite":
                continue
        ...
        "graphs": [
            {
                ...
                "ambiguous_title_mappings": [{"title": t, "type_ids": list(ids)} for (t, ids) in g.ambiguous_title_mappings],
