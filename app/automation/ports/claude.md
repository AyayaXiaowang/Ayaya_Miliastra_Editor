...
- `port_picker.py`：在识别到的端口列表中选择合适的端口中心，侧别/可连接性与类型筛选统一委托 `_ports.filter_ports_for_screen_candidates` 作为单一入口，其余多级回退策略围绕命名、序号、数字端口名与索引等信息按固定优先级顺序依次尝试；端口几何排序与“期望为 flow/data 时的种类校验”基于 `_ports.get_port_center_y()` 与 `_ports.get_port_category()`，并通过内部 `_pick_candidate_with_kind_preference` 工具在命名/索引等策略中统一实现“优先选同 kind，再回退到首个候选”的逻辑；当命名优先未命中时会额外打印候选摘要（name/side/kind/index/center/bbox），用于排查识别漂移或候选污染；日志输出则通过 `app.automation.editor.executor_utils.make_executor_log_fn` 构造基于 `executor._log(..., log_callback)` 的闭包，保持各策略日志前缀与回调使用方式一致；当某一帧端口识别结果为空且运行在真实编辑器执行器环境下时，会优先基于节点中心在画布区域内寻找安全空白点，将鼠标移出节点到该空白位置后重新截图并重试一次端口识别，以降低因鼠标遮挡或节点局部 UI 状态导致的“无端口候选”失败。
...
- `dict_port_type_steps.py`：封装字典端口类型设置步骤，通过 Dictionary 图标与“键/值”标签模板完成键/值类型选择，并与画布交互/吸附工具协同；相关模板路径由执行器的 `ocr_template_profile` 决定（避免硬编码 `4K-CN`）；内部按“确认 Dictionary 图标与必要的类型切换 / 打开字典设置对话框 / 应用键/值类型 / 点击画布空白结束配置”拆分为职责单一的助手函数；键/值标签点击后的“类型搜索 + 选择”流程复用 `port_type_ui_core.apply_type_in_open_search_dialog`，在单次字典配置过程中共享对话框截图与模板匹配结果，减少重复 OCR/模板匹配；当端口尚未被声明为“字典”时，会通过 `port_type_ui_core.set_port_type_with_settings` 先将端口类型切换为基础“字典”，再重新截图查找 Dictionary 图标，避免在高层步骤模块之间引入循环依赖；与其它端口类型 UI 步骤一致，日志回调与暂停/终止钩子等运行时上下文统一经 `app.automation.editor.executor_protocol.AutomationStepContext` 传递，避免在多个 helper 之间重复维护参数组合。
...
- `variadic_ports.py` / `dict_ports.py`：变参端口和字典端口的新增与管理，共用 `_add_ports_common.execute_add_ports_generic()` 完成节点解析、最终新增数量计算与 Add 图标点击；变参端口在统计当前端口数量时统一复用 `NodePortsSnapshotCache` 提供的节点端口快照，并结合 `_ports.is_data_input_port` 与端口 `name_cn` 的数字匹配筛选左侧数字数据输入行作为变参端口集合，再依据 Todo 中的目标数量计算最终新增数量；字典端口则直接使用 Todo 中给定的 `add_count`，二者在日志前缀与 `prefer_multi` 策略上保持一致。
  - 节点定位失败时，`port_type_setter.py` 会根据当前程序坐标绘制期望位置矩形并在监控面板上高亮；位置容差与搜索范围由节点几何基准（profile 推导）决定，便于在不同分辨率/缩放下保持一致的定位行为。
...
- `port_type_resolver.py`：无 UI 依赖的“端口有效类型（展示级）解析”入口；核心推断规则收敛在引擎侧 `engine.graph.port_type_effective_resolver`，本模块仅做 executor（NodeDef 获取）适配与兜底返回值处理，供画布与 Todo 侧复用。
  - 支持 `fail_on_generic=True`：当端口有效类型仍为“泛型家族”（含空/泛型列表/泛型字典等占位）时直接抛错，用于 fail-closed 场景（例如只读画布展示/严格校验/导出链路），避免静默回退掩盖类型实例化缺失。
  - 有效类型解析以 `EffectivePortTypeResolver.resolve(...)` 的结果为准；仅当引擎侧无法给出结果时才回退到 `NodeModel.input_types/output_types`。这样可避免常量回填等因素导致 `input_types` 呈现为“字符串”时，覆盖掉正确的有效类型推断结果，确保画布与任务清单展示口径一致。
  - 内部缓存：按 `GraphModel` + `edges_revision` + `executor` 身份缓存 `EffectivePortTypeResolver`（含边索引与 node_def 缓存），避免在画布端口布局/大图加载时为每个端口重复扫描全图连线导致 UI 线程阻塞；当连线通过 `GraphModel` API 变更或调用 `touch_edges_revision()` 后会自动失效并重建。
...
- `port_type_steps_common.py`：封装输入/输出侧复用的端口遍历与类型设置 UI 骨架（在已有快照上按端口名定位中心并调用具体设置回调），统一 `typed_side_once` 与“首个数据端口复用当前帧快照”的策略；当端口中心定位失败或 Settings 类型设置失败时会返回失败，避免“未实际设置但步骤被判定成功”，输入/输出侧在存在“需要设置类型的端口”但最终未成功设置任何类型时也会返回失败，交由执行线程触发回退重试与后续跳过策略。
- `port_type_steps_input.py`：当步骤提供 `params_list` 时，会将其中出现且确认为节点输入端口的名字视为“期望输入端口集合”，并据此控制本次只对声明过的输入端口尝试类型设置；对运算节点启用“同侧仅设置一次”的优化策略后，会将同侧剩余期望输入端口视为已满足，避免因提前停止遍历导致的“已设置一次但仍提示缺少期望端口名”的假失败；端口序号→端口名映射统一走 `map_port_index_to_name_via_node_def`（已持有唯一 node_def），不再使用 `engine.nodes.port_index_mapper` 的“按节点中文名全库反查”路径。
