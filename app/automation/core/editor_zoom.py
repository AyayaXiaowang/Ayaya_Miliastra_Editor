...
def ensure_zoom_ratio_50(
    ...
    if allow_continue is not None and not allow_continue():
        executor._log("用户终止/暂停，放弃缩放检查", log_callback)
        return False

    screenshot = editor_capture.capture_window(executor.window_title)
    if not screenshot:
        executor._log("✗ 截图失败（缩放检查）", log_callback)
        return False
    ...
    def _click_editor_xy(ex: int, ey: int, label: str) -> None:
        ...
        executor._log(f"[鼠标] 点击 {label}：screen=({int(sx)},{int(sy)})", log_callback)
    ...
    if val0 is None:
        ...
        rects1 = [ { 'bbox': (int(region_x), int(region_y), int(region_w), int(region_h)), 'color': (120, 200, 255), 'label': '缩放区域(激活后)' } ]
        if details1:
            for d in details1:
                ...
                if isinstance(box_d, (list, tuple)) and len(box_d) == 4:
                    if isinstance(box_d[0], (list, tuple)):
                        ...
                        bw = max(1, int(rx - lx)); bh = max(1, int(by - ty))
                        rects1.append({ 'bbox': (int(region_x + lx), int(region_y + ty), int(bw), int(bh)), 'color': (0, 200, 0), 'label': str(text_d) })
                    else:
                        lx = float(box_d[0]); ty = float(box_d[1]); rx = float(box_d[2]); by = float(box_d[3])
                        rects1.append({ 'bbox': (int(region_x + lx), int(region_y + ty), int(max(1, rx - lx)), int(max(1, by - ty))), 'color': (0, 200, 0), 'label': str(text_d) })
        executor._emit_visual(screenshot, { 'rects': rects1 }, visual_callback)

    if val0 is None:
        executor._log("✗ 缩放未识别为数值", log_callback)
        return False
        ...
        for d in details_up:
            ...
            if isinstance(box_d, (list, tuple)) and len(box_d) == 4:
                if isinstance(box_d[0], (list, tuple)):
                    ...
                    lx = float(box_d[0]); ty = float(box_d[1]); rx = float(box_d[2]); by = float(box_d[3])
                    rects_up.append({ 'bbox': (int(sx0 + lx), int(sy0 + ty), int(max(1, rx - lx)), int(max(1, by - ty))), 'color': (0, 200, 0), 'label': str(text_d) })
    executor._emit_visual(screenshot, { 'rects': rects_up }, visual_callback)
    ...
    if chosen_bbox_up is not None:
        ...
        executor._emit_visual(screenshot, { 'circles': circles_up }, visual_callback)
        ...
        for d in details2:
            ...
            if isinstance(box_d, (list, tuple)) and len(box_d) == 4:
                if isinstance(box_d[0], (list, tuple)):
                    ...
                    lx = float(box_d[0]); ty = float(box_d[1]); rx = float(box_d[2]); by = float(box_d[3])
                    rects_final.append({ 'bbox': (int(region_x + lx), int(region_y + ty), int(max(1, rx - lx)), int(max(1, by - ty))), 'color': (0, 200, 0), 'label': str(text_d) })
    executor._emit_visual(screenshot, { 'rects': rects_final }, visual_callback)
    return bool(val2 == 50)
