...
class PackageController(QtCore.QObject):
    ...
    def save_package(self) -> None:
        """保存存档"""
        # 在真正落盘前，先刷新属性面板中尚未写回模型的基础信息（名称/描述/GUID 等）。
        if self.flush_current_resource_panel is not None:
            self.flush_current_resource_panel()
        print(f"[PACKAGE-SAVE] 开始保存存档: package_id={self.current_package_id!r}")

        # 特殊视图模式下保存当前编辑的资源
        if self.current_package_id in ("global_view", "unclassified_view"):
            self.request_save_current_graph.emit()
            self._save_global_view_resources()
            self._save_management_for_special_view()
            print(
                "[PACKAGE-SAVE] 已保存全局/未分类视图下的当前资源，"
        ...
        # 保存所有修改的资源到 ResourceManager
        self._save_package_resources()

        # 将当前包的战斗预设 / 信号 / 管理配置从视图模型回写到 PackageIndex 与资源库
        if isinstance(self.current_package, PackageView):
            self._sync_combat_presets_to_index(self.current_package, self.current_package_index)
            self._sync_signals_to_index(self.current_package, self.current_package_index)
            self._sync_management_resources_to_index(self.current_package, self.current_package_index)
        
        # 更新时间
        self.current_package_index.updated_at = datetime.now().isoformat()
        
        # 保存存档索引
        self.package_index_manager.save_package_index(self.current_package_index)
    ...
    def _save_global_view_resources(self) -> None:
        """保存全局视图模式下修改的资源"""
    ...
    def _save_management_for_special_view(self) -> None:
    ...
    def _save_single_config_save_points(self, raw_value) -> None:
        """在全局/未分类视图下，将 management.save_points 写回全局元配置。

        设计约定：
        - 仅负责写回 ID 为 'global_view_save_points' 的 SAVE_POINT 资源；
        - 模板本体以代码级资源形式存放在 `assets/资源库/管理配置/局内存档管理/` 中，
          本方法不再为每个模板创建或删除 JSON 资源文件；
    ...
    def _save_package_resources(self) -> None:
        """保存存档中所有修改的资源到 ResourceManager"""
    ...
    def _sync_combat_presets_to_index(self, package: PackageView, package_index: PackageIndex) -> None:
    ...
    def _sync_management_resources_to_index(self, package: PackageView, package_index: PackageIndex) -> None:
