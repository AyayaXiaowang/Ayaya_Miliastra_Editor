...
from app.common.decorations_merge import merge_instance_decorations
...
class EntityPlacementWidget(
    ...
    def _ensure_merge_carrier_template(self) -> str:
        """确保当前项目存档中存在一个“空载体”元件用于承载装饰物组。

        约定：使用稳定 ID `shape_editor_empty__<package_id>`，避免重复创建多个空载体元件。
        """
    ...
    def _merge_decorations_into_one_instance(self) -> None:
        """将多个实体的 decorations 合并到同一个目标实体（项目资源级）。"""
        if not isinstance(self.current_package, PackageView):
        ...
        # 仅允许选择“当前项目存档目录”下的实体摆放条目（不含共享根目录资源）。
        ...
        for instance_id, instance in package.instances.items():
            ...
            # compute decorations count from instance.metadata
            deco_count = 0
            ...
            if isinstance(meta, dict):
                ...
                if isinstance(decos, list):
                    deco_count = int(len(decos))

            template = package.get_template(getattr(instance, "template_id", "") or "")
            resolved_entity_type = ""
            template_name = ""
            if template is not None:
                resolved_entity_type = str(getattr(template, "entity_type", "") or "").strip()
            ...
            name_text = str(getattr(instance, "name", "") or "").strip() or instance_id
        ...
        # Build source instances list (exclude target itself to avoid duplication when keeping target existing).
        source_instances: list[InstanceConfig] = []
        target_id_text = str(getattr(target_instance, "instance_id", "") or "").strip()
        ...
        # Optionally remove sources from current package (move out of package dir).
        ...
        if dialog.should_remove_sources():
            ...
            if not isinstance(package_index_manager, PackageIndexManager):
                ...
                for sid in source_ids:
                    if sid == updated_target.instance_id:
                        continue
