...
class TodoExecutorBridge(QtCore.QObject):
    ...
    def _ensure_execution_backend_available(self) -> bool:
        """门禁：OCR 依赖缺失时禁用真实执行入口，避免点击后因导入失败而炸穿。"""
        if bool(self.ui_context.is_ocr_available()):
            return True

        message = (
            "未检测到 rapidocr_onnxruntime（OCR 依赖）：已禁用『执行/自动化』功能。\n"
            "你仍可正常浏览任务清单与节点图；如需执行，请先安装 OCR 依赖并重启程序。"
        )
        self._notify(message, "warning")

        monitor = self.ui_context.try_get_execution_monitor()
        if monitor is not None:
            monitor.update_status("执行不可用（未安装 OCR）")
            monitor.log("⚠ 未检测到 rapidocr_onnxruntime：本环境无法使用执行/识别功能。")
        return False
