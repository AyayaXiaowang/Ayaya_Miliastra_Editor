...
from app.ui.todo.preview.todo_preview_panel import TodoPreviewPanel
from app.ui.todo.detail.todo_detail_panel import TodoDetailPanel
from app.ui.todo.tree.todo_tree import TodoTreeManager
from app.ui.todo.runtime.todo_executor_bridge import TodoExecutorBridge
from app.ui.todo.list.todo_context_menu import TodoContextMenu
from app.ui.todo.runtime.todo_runtime_state import TodoRuntimeState
...
class TodoListOrchestrator:
    ...
    def show_detail(self, todo: TodoItem) -> None:
        ...
        # OCR 缺失时允许任务清单正常浏览，但禁用真实执行入口（避免点击后因导入 OCR 依赖失败而炸穿）。
        execution_backend_available = bool(self.ui_context.is_ocr_available())
        effective_executable = bool(execution_profile.is_executable) and bool(execution_backend_available)

        host.detail_panel.set_execute_visible(effective_executable)
        host.preview_panel.set_execute_visible(effective_executable)
        ...
        show_exec_remaining = bool(execution_profile.supports_execute_remaining) and bool(execution_backend_available)
        ...
        if monitor_panel is not None:
            ...
            if getattr(host, "execution_compact_mode_enabled", False):
                ...
                monitor_panel.set_execute_visible(bool(effective_executable))
