    ...
    def test_bool_enum_options(self) -> None:
        ...
        for visible_node in visible_nodes:
            ...
            if len(debug_log_lines) < 20:
                ports_brief = format_candidates_brief(
                    filter_screen_input_candidates(ports_snapshot, expected_kind=None)
                )
                debug_log_lines.append(
                    f"· 节点 '{node_title}'：端口识别={len(ports_snapshot)}；候选(左侧/不限kind)={ports_brief}"
                )
            ...
            for node_def in candidate_defs:
                ...
                for input_index, port_name in enumerate(input_names_in_order):
                    ...
                    if is_bool_or_enum:
                        bool_enum_ports_by_index.append(
                            (int(input_index), port_name_text, declared_type_text, bool(is_boolean))
                        )
                ...
                if len(debug_log_lines) < 30:
                    bool_enum_preview = ", ".join(
                        [
                            f"{idx}:{name}[{tp}]"
                            for idx, name, tp, _is_bool in bool_enum_ports_by_index[:8]
                        ]
                    )
                ...
                for input_index, port_name_text, declared_type_text, _is_boolean in bool_enum_ports_by_index:
                    ...
                    if chosen_port_obj is None:
                        ...
                        ordered_left_ports = filter_screen_input_candidates(
                            ports_snapshot, expected_kind=None
                        )
        ...
        if (
            ...
            for entry in debug_node_entries:
                ...
                for port_obj in ports_preview:
                    ...
                    rects.append(
                        {
                            "bbox": port_bbox,
                            "color": (0, 220, 0) if highlight_bool_enum else ((255, 160, 80) if port_side == "right" else (0, 200, 140)),
            ...
            if debug_log_lines:
                for line in debug_log_lines[:30]:
                    self._log(line)
            self._log(
                "✗ 布尔/枚举选项测试失败：未能在当前画面定位到任何布尔/枚举端口（请先让带布尔/枚举端口的节点出现在视口中）"
            )
            return
        ...
        click_screen_x, click_screen_y = executor.convert_editor_to_screen_coords(
            click_editor_x, click_editor_y
        )
        ...
        self._update_visual(
            screenshot_before,
            {
                "rects": [
                    {
                        "bbox": selected_node_bbox,
                        "color": (120, 200, 255),
                        "label": str(selected_node_title),
                    }
                ]
                + [
                    {
                        "bbox": selected_port_bbox,
                        "color": (0, 200, 140),
                        "label": f"端口bbox {selected_port_name}",
                    }
                ],
                "circles": [
                    {
        ...
        for rect_x, rect_y, rect_w, rect_h, distance in found_rectangles:
            ...
            if int(click_editor_x) < int(rect_x) - 60 or int(click_editor_x) > int(
                rect_x + rect_w
            ) + 60:
                continue
