...
- `retry_handler.py`：收敛任务执行过程中的失败重试策略，维护最近成功锚点并在需要时回退重试；重试优先采用“当前位置识别刷新坐标映射→重试步骤”，当识别无法建立稳定映射时启用“逐步回退（每次1步向锚点方向拖拽）→每步后尝试识别恢复映射→识别恢复即重试步骤”的兜底流程；若逐步回退仍失败，再执行一次完整回退并再次尝试识别；视口操作通过注入的 `ViewportController` 实现，避免直接访问执行器私有方法。
