...
用法:
    python -X utf8 app/cli/convert_graph_to_executable.py 资源库/节点图/server/xxx.py
...
from engine import GraphCodeParser, ExecutableCodeGenerator, get_node_registry, log_info, log_error
...
def main() -> int:
    if len(sys.argv) < 2:
        log_error("用法: python -X utf8 app/cli/convert_graph_to_executable.py <节点图代码文件路径>")
        log_error("\n示例:")
        log_error("  python -X utf8 app/cli/convert_graph_to_executable.py 资源库/节点图/server/xxx.py")
        return 1
