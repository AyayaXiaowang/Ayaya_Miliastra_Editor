...
## 依赖边界
- 允许依赖：`engine/*`、`plugins/*`、`app/runtime/*`（通过 `app.runtime.*` 正式导入路径使用）
- 禁止依赖：`app/ui/*`（避免把 UI 逻辑引入生成器）、`core/*`

## 当前状态
- 节点图导出与复合节点导出的“可执行/可运行”代码生成器已迁入本目录；`engine` 不再包含运行时绑定的生成逻辑。
- 生成代码默认采用 **workspace bootstrap + `app.runtime.engine.graph_prelude_*`** 的导入策略（不再依赖资源库内的本地 prelude 文件）：上层可通过参数选择导入模式、选择 server/client 预设以及是否启用 `@validate_node_graph`（校验入口默认指向 `engine.validate.node_graph_validator`）。
