...
- 运行时节点函数导出：由 `app.runtime.engine.graph_prelude_server` / `app.runtime.engine.graph_prelude_client` 基于 V2 AST 清单加载并注入；`plugins.nodes.server` / `plugins.nodes.client` 包级 `__init__.py` 为无副作用最小包。
- `server/`、`client/` 下的节点实现文件通过 `@node_spec` 声明元信息与端口类型；`shared/` 存放 helper。

## 注意事项
- 依赖边界：允许依赖 `engine/*`；禁止依赖 `app/*`（除运行时装配）与 `assets/*`（除只读数据）。
