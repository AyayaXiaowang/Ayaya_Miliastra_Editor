...
- 资源库共享目录规范：`docs/资源库_共享目录规范.md`（共享只放可复用稳定资源；测试/回归资源必须放入测试项目存档）。
- Cursor CLI/Agent 使用指南：`docs/Cursor_CLI_Agent_使用指南.md`（安装、模式、print 脚本化、Rules/MCP/Cloud Agent）。
- 面向使用者的入口 `docs/用户必读.md` 默认以“资源库包化目录结构”为准：`assets/资源库/共享/` + `assets/资源库/项目存档/<package_id>/`（其中实体摆放目录为 `实体摆放/`）。
- 新人速查：`docs/GIA_GIL_新手指引_收到文件怎么读.md`（收到用户发来的 `.gia/.gil`，先用工具转 Graph IR / readable JSON 再阅读；做测试/逆向分析时优先使用“当前代码现导”的最新产物并附导出命令；包含信号专项诊断 `inspect_gil_signals` 的用法）。
- `.gil` 对照指南：`docs/GIA_GIL_diff_two_gils.md`（两份 `.gil` 的 payload Graph IR diff；用于定位“游戏处理后导出 vs 工具直接处理”的结构差异）。
...
- `.gia` 导出入口梳理与收敛方案：`docs/GIA导出链路_收敛与兼容方案.md`（按资产类型分类入口与底层实现，并维护 entrypoint→canonical 实现的映射，明确“门面层 + 兼容壳”的非破坏收敛策略）。
...
- Cursor Hooks 开发指南：`docs/Cursor_Hooks_开发指南.md`（本仓库 hooks 的 stdin/stdout/stderr 契约、回归命令与新人写 hook 的最短闭环）。
- Cursor hooks 校验改造方案：`docs/Cursor_Hooks_校验改造方案.md`（把节点图/UI/项目存档校验分层落到 afterFileEdit 与 git commit 前拦截，给出脚本与配置的具体改造方案）。
