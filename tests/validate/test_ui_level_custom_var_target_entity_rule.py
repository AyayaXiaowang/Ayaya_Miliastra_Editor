...
def test_lv_ui_custom_var_should_work_for_non_ui_prefix_var_name(tmp_path: Path) -> None:
    """回归：UI 占位符引用到的 lv 变量名不以 ui_ 开头时，也应参与目标实体归属校验。"""
    workspace = _workspace_root()
    package_root = workspace / "assets" / "资源库" / "项目存档" / "__tmp_validate_ui_lv_custom_var_target__"
    ui_source_dir = package_root / "管理配置" / "UI源码"
    ui_source_dir.mkdir(parents=True, exist_ok=True)
    (ui_source_dir / "test.html").write_text(
        '<div class="x">{1:lv.UI选关_文本.some_key}</div>\n',
        encoding="utf-8",
    )

    graph_dir = package_root / "节点图" / "server" / "实体节点图"
    graph_dir.mkdir(parents=True, exist_ok=True)
    graph_path = graph_dir / "temp_ui_lv_custom_var_target_non_ui_prefix_error.py"
    graph_path.write_text(
        '''
"""
graph_id: test_ui_lv_custom_var_target_non_ui_prefix_error
graph_name: UI关卡变量_非ui前缀_写到owner_entity但未声明_应报错
graph_type: server
"""
...
class UI关卡变量_非ui前缀_写到owner_entity但未声明_应报错:
    def __init__(self, game, owner_entity):
        self.game = game
        self.owner_entity = owner_entity

    def on_界面控件组触发时(self, 事件源实体, 事件源GUID, 界面控件组组合索引, 界面控件组索引):
        v: "字符串-字符串字典" = 获取自定义变量(self.game, 目标实体=self.owner_entity, 变量名="UI选关_文本")
        设置自定义变量(self.game, 目标实体=self.owner_entity, 变量名="UI选关_文本", 变量值=v, 是否触发事件=False)
''',
        encoding="utf-8",
    )
