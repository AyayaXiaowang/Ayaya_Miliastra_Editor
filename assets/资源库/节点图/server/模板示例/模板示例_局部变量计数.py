...
class 模板示例_局部变量计数:
    ...
    def on_实体创建时(self, 事件源实体, 事件源GUID):
        ...
        for 轮次索引 in range(0, 30):
            本轮摇值: "整数" = 获取随机整数(self.game, 下限=0, 上限=2)
            设置局部变量(self.game, 局部变量=最近一次摇值句柄, 值=本轮摇值)
            最近一次摇值 = 本轮摇值

            命中一次 = 是否相等(self.game, 枚举1=本轮摇值, 枚举2=1)
            if 命中一次:
                新命中次数: "整数" = 加法运算(self.game, 左值=当前命中次数, 右值=1)
                设置局部变量(self.game, 局部变量=命中计数句柄, 值=新命中次数)
                当前命中次数 = 新命中次数
