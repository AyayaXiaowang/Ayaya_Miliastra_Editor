...
graph_type: server
description: 示例节点图（结构体拼装/拆分/修改）：使用基础结构体 `全类型结构体示例` 演示【拼装结构体】生成实例并写入自定义变量、【拆分结构体】读取字段并镜像到节点图变量、以及在【自定义变量变化时】通过【修改结构体】更新单个字段并写回。
"""
...
GRAPH_VARIABLES: list[GraphVariableConfig] = [
    GraphVariableConfig(
        ...
        variable_type="结构体",
        default_value=None,
        description="记录最近一次通过【拼装结构体】构造的结构体实例，便于在编辑器中查看结构体字段值。",
        is_exposed=False,
    ),
    GraphVariableConfig(
        name="调试_整数字段镜像",
        variable_type="整数",
        default_value=0,
        description="镜像 全类型结构体示例.整数字段 的数值，便于在编辑器中观察。",
        is_exposed=False,
    ),
    GraphVariableConfig(
        name="调试_字符串字段镜像",
        variable_type="字符串",
        default_value="",
        description="镜像 全类型结构体示例.字符串字段 的文本内容。",
        is_exposed=False,
    ),
    GraphVariableConfig(
        name="调试_布尔值字段镜像",
        variable_type="布尔值",
        default_value=False,
        description="镜像 全类型结构体示例.布尔值字段 的布尔状态。",
        is_exposed=False,
    ),
]
...
class 模板示例_结构体全类型_拆分与拼装:
    ...
    用法约定：
    - 结构体定义 `全类型结构体示例` 已在“管理配置/结构体定义”中作为【基础结构体】存在；
    ...
    def on_实体创建时(self, 事件源实体, 事件源GUID):
        ...
        # 在真实节点图中，此处对应一颗绑定 `全类型结构体示例` 的【拼装结构体】节点，
    ...
    def on_自定义变量变化时(
        ...
        实体字段_拆分, GUID字段_拆分, 整数字段_拆分, 布尔值字段_拆分, 浮点数字段_拆分, 字符串字段_拆分 = 拆分结构体(
            self.game,
            结构体名="全类型结构体示例",
            结构体=当前结构体实例,
        )
        实体字段_修改前: "实体" = 实体字段_拆分
        GUID字段_修改前: "GUID" = GUID字段_拆分
        整数字段_修改前: "整数" = 整数字段_拆分
        布尔值字段_修改前: "布尔值" = 布尔值字段_拆分
        浮点数字段_修改前: "浮点数" = 浮点数字段_拆分
        字符串字段_修改前: "字符串" = 字符串字段_拆分
        # 注意：禁止通过赋值语句将“命名常量”复制到变量（会触发 CODE_NO_CONST_ALIAS_ASSIGNMENT）。
        # 这里直接使用【拆分结构体】的输出常量参与后续计算即可。
        新整数字段值: "整数" = 整数字段_修改前 + 1

        # 在真实节点图中，这一步对应一颗绑定 `全类型结构体示例` 的【修改结构体】节点，
        ...
        实体字段_拆分2, GUID字段_拆分2, 整数字段_拆分2, 布尔值字段_拆分2, 浮点数字段_拆分2, 字符串字段_拆分2 = 拆分结构体(
            self.game,
            结构体名="全类型结构体示例",
            结构体=写回后结构体实例,
        )
        实体字段_修改后: "实体" = 实体字段_拆分2
        GUID字段_修改后: "GUID" = GUID字段_拆分2
        整数字段_修改后: "整数" = 整数字段_拆分2
        布尔值字段_修改后: "布尔值" = 布尔值字段_拆分2
        浮点数字段_修改后: "浮点数" = 浮点数字段_拆分2
        字符串字段_修改后: "字符串" = 字符串字段_拆分2
        设置节点图变量(
            self.game,
            变量名="调试_整数字段镜像",
            变量值=整数字段_修改后,
