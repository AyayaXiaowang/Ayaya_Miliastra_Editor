...
graph_type: server
description: 示例节点图（踏板开关：信号广播版）：踏板端在“按下完成/回弹开始”广播 `通用踏板开关_状态变化(是否激活=...)`；接收端通过监听事件参数 `信号来源实体` 识别发送方并驱动联动目标；支持最小触发人数与相对位移方向配置；若需要一次性锁定，由接收端回发 `通用踏板开关_激活确认` 触发踏板锁定并禁用触发器。
"""
...
class 模板示例_踏板开关_信号广播:
    ...
    def on_实体创建时(self, 事件源实体, 事件源GUID):
        ...
        下沉方向向量: "三维向量" = 右上合成 + 前分量向量

        位移向量: "三维向量" = 下沉方向向量 * 位移距离
        目标位置: "三维向量" = 原始位置 + 位移向量
        设置节点图变量(
            self.game,
            变量名="按下目标位置",
    ...
    def on_离开碰撞触发器时(self, 离开者实体, 离开者实体GUID, 触发器实体, 触发器实体GUID, 触发器序号):
        ...
        if 触发器序号 == 目标序号:
            ...
            是否处于按下状态_离开: "布尔值" = 当前目标状态_离开 == "按下"
            是否人数不足: "布尔值" = 当前数量_离开 < 最小触发人数_离开
            if 是否处于按下状态_离开:
                if 是否人数不足:
