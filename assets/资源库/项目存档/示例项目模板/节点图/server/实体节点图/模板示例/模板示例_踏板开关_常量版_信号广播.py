...
graph_type: server
description: 示例节点图（踏板开关：常量版信号广播）：踏板端在“按下完成/回弹开始”广播 `通用踏板开关_状态变化(是否激活=...)`。本版本把位移距离/速度/最小触发人数/触发器序号/相对位移方向/一次性开关/特效等写成字面量常量，不对外暴露为节点图变量；仅保留少量隐藏节点图变量用于跨事件保存运行期状态。
"""
...
GRAPH_VARIABLES: list[GraphVariableConfig] = [
    ...
    GraphVariableConfig(
        ...
        variable_type="布尔值",
        default_value=False,
        description="常量开关（隐藏）：为 True 时在接收端回发确认后锁定按下并禁用触发器，不再回弹。",
        is_exposed=False,
    ),
]
...
class 模板示例_踏板开关_常量版_信号广播:
    ...
    def on_实体创建时(self, 事件源实体, 事件源GUID):
        ...
        右分量向量: "三维向量" = 本地右向量 * 方向X
        上分量向量: "三维向量" = 本地上向量 * 方向Y
        前分量向量: "三维向量" = 本地前向量 * 方向Z

        右上合成: "三维向量" = 右分量向量 + 上分量向量
        位移方向向量: "三维向量" = 右上合成 + 前分量向量

        位移向量: "三维向量" = 位移方向向量 * 位移距离
        目标位置: "三维向量" = 原始位置 + 位移向量
        设置节点图变量(
            self.game,
            变量名="按下目标位置",
    ...
    def on_进入碰撞触发器时(self, 进入者实体, 进入者实体GUID, 触发器实体, 触发器实体GUID, 触发器序号):
        ...
        是否目标触发器: "布尔值" = 触发器序号 == 目标序号
        if 是否目标触发器:
            当前目标状态: "字符串" = 获取节点图变量(
                self.game,
                变量名="运动目标状态",
            )
            是否已处于按下状态: "布尔值" = 当前目标状态 == "按下"
    ...
    def on_离开碰撞触发器时(self, 离开者实体, 离开者实体GUID, 触发器实体, 触发器实体GUID, 触发器序号):
        ...
        if 是否目标触发器:
            ...
            最小触发人数: "整数" = 1
            是否人数不足: "布尔值" = 当前数量 < 最小触发人数

            当前目标状态: "字符串" = 获取节点图变量(
                self.game,
                变量名="运动目标状态",
            )
            是否处于按下状态: "布尔值" = 当前目标状态 == "按下"
    ...
    def on_通用踏板开关_激活确认(
        ...
        是否给本踏板: "布尔值" = 开关GUID == 自身GUID
        是否允许锁定_布尔: "布尔值" = 是否允许锁定

        # 常量：是否一次性（隐藏节点图变量，默认 False；需要时可在编辑器中覆盖默认值）
        是否一次性: "布尔值" = 获取节点图变量(self.game, 变量名="是否一次性")
