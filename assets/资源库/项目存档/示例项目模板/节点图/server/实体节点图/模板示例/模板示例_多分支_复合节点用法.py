...
graph_type: server
description: 示例节点图（复合节点：多分支）：在【实体创建时】调用“多分支示例复合节点”，根据随机整数从不同流程出口流出；在 Graph Code 中用 `match self.多分支_示例_类格式.按整数多分支(...)` 将多个流程出口分别连接到不同的后续逻辑/调试写回。
"""
...
class 模板示例_多分支_复合节点用法:
    ...
    2. 在【实体创建时】事件中：
       - 生成 0~3 的随机整数作为分支值；
       - 将该值写入图变量“调试_最后一次分支值”；
       - 使用 `match self.多分支_示例_类格式.按整数多分支(分支值=分支值)`，根据复合节点的不同流程出口（“分支为0/分支为1/分支为其他”）分别执行不同的后续逻辑，并写入图变量“调试_最近分支标签”。
