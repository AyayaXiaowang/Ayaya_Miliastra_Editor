...
def main() -> None:
    ...
    template_ids = resource_manager.list_resources(ResourceType.TEMPLATE)

    loaded_count = 0
    for template_id in template_ids:
        payload = resource_manager.load_resource(ResourceType.TEMPLATE, template_id)
    ...
    print("元件库 模板资源校验")
    print("=" * 60)
    print(f"模板总数量: {len(template_ids)}")
    print(f"成功加载数量: {loaded_count}")
    print("元件库 JSON 资源格式与引擎读取约定一致。")
