from __future__ import annotations

from engine.resources.auto_custom_variable_registry import (
    AutoCustomVariableDeclaration,
)

# 本文件：自定义变量“统一声明入口”（测试基础内容）。
# - 不执行运行期逻辑；仅做静态声明。
# - owner：直接填实体/元件 ID 或 player/level（支持列表形式的多 owner）。

CUSTOM_VARIABLE_DECLARATIONS: list[AutoCustomVariableDeclaration] = [
    AutoCustomVariableDeclaration(
        variable_id='var_sample_integer_counter__测试基础内容',
        variable_name='示例_计数器_整数',
        variable_type='整数',
        default_value=0,
        description='示例：全局整数变量',
        owner='level',
        category='教学示例',
    ),
    AutoCustomVariableDeclaration(
        variable_id='var_sample_float_ratio__测试基础内容',
        variable_name='示例_倍率_浮点数',
        variable_type='浮点数',
        default_value=1.25,
        description='示例：全局浮点数变量',
        owner='level',
        category='教学示例',
    ),
    AutoCustomVariableDeclaration(
        variable_id='var_sample_boolean_switch__测试基础内容',
        variable_name='示例_开关_布尔值',
        variable_type='布尔值',
        default_value=False,
        description='示例：全局布尔开关',
        owner='level',
        category='教学示例',
    ),
    AutoCustomVariableDeclaration(
        variable_id='var_sample_string_note__测试基础内容',
        variable_name='示例_备注_字符串',
        variable_type='字符串',
        default_value='示例默认文本',
        description='示例：全局字符串变量',
        owner='level',
        category='教学示例',
    ),
    AutoCustomVariableDeclaration(
        variable_id='var_sample_vector3_spawn__测试基础内容',
        variable_name='示例_出生点_三维向量',
        variable_type='三维向量',
        default_value=(0.0, 0.0, 0.0),
        description='示例：三维向量（位置/方向）变量',
        owner='level',
        category='教学示例',
    ),
    AutoCustomVariableDeclaration(
        variable_id='var_sample_guid__测试基础内容',
        variable_name='示例_GUID',
        variable_type='GUID',
        default_value='1073742153',
        description='示例：GUID（数字字符串）',
        owner='level',
        category='教学示例',
    ),
    AutoCustomVariableDeclaration(
        variable_id='var_sample_integer_list__测试基础内容',
        variable_name='示例_整数列表',
        variable_type='整数列表',
        default_value=[1, 2, 3],
        description='示例：整数列表',
        owner='level',
        category='教学示例',
    ),
    AutoCustomVariableDeclaration(
        variable_id='var_sample_string_list__测试基础内容',
        variable_name='示例_字符串列表',
        variable_type='字符串列表',
        default_value=['条目A', '条目B', '条目C'],
        description='示例：字符串列表',
        owner='level',
        category='教学示例',
    ),
    AutoCustomVariableDeclaration(
        variable_id='var_sample_boolean_list__测试基础内容',
        variable_name='示例_布尔值列表',
        variable_type='布尔值列表',
        default_value=[True, False, True],
        description='示例：布尔值列表',
        owner='level',
        category='教学示例',
    ),
    AutoCustomVariableDeclaration(
        variable_id='var_sample_vector3_list__测试基础内容',
        variable_name='示例_三维向量列表',
        variable_type='三维向量列表',
        default_value=[(0.0, 0.0, 0.0), (1.0, 2.0, 3.0)],
        description='示例：三维向量列表',
        owner='level',
        category='教学示例',
    ),
    AutoCustomVariableDeclaration(
        variable_id='var_sample_flat_dict__测试基础内容',
        variable_name='示例_界面展示_字典(一层)',
        variable_type='字典',
        default_value={'当前钻石': 0, '当前金币': 0, '当前提示': '示例提示文本', '是否开启': False, '当前计数': 0},
        description='示例：字典类型（仅允许一层键值表，不在 value 中再嵌套字典）',
        owner='level',
        category='教学示例',
    ),
    AutoCustomVariableDeclaration(
        variable_id='var_sample_struct_list__测试基础内容',
        variable_name='示例_结构体列表(基础结构体)',
        variable_type='结构体列表',
        default_value={'struct_id': '全类型结构体示例', 'items': [{'name': '条目1', 'fields': {'整数字段': '1', '布尔值字段': 'True', '浮点数字段': '1.5', '字符串字段': '示例字符串', 'GUID字段': '1073742154', '三维向量字段': '0,0,0'}}]},
        description='示例：结构体列表（引用基础结构体 全类型结构体示例；字段值统一使用字符串）',
        owner='level',
        category='教学示例',
    ),
    AutoCustomVariableDeclaration(
        variable_id='var_sample_level1_passed__测试基础内容',
        variable_name='关卡1已通过',
        variable_type='布尔值',
        default_value=False,
        description='示例：玩家是否已通过关卡1（踏板开关触发双开门后写入 True）。',
        owner='level',
        category='教学示例',
    ),
]
