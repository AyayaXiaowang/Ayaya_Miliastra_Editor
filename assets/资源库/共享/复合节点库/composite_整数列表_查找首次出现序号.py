"""
composite_id: composite_整数列表_查找首次出现序号
node_name: 整数列表_查找首次出现序号
node_description: 返回目标值在整数列表中首次出现的序号（未找到返回-1），并输出是否找到
scope: server
"""

# Python 等价写法：
# - 序号 = 输入列表.index(目标值)  # 未找到会抛异常；本复合节点改为返回 -1
# - 是否找到 = (序号 != -1)
#
# 示例输入输出：
# - 输入列表=[5, 8, 5], 目标值=5 -> 是否找到=True, 首次出现序号=0
# - 输入列表=[5, 8, 5], 目标值=9 -> 是否找到=False, 首次出现序号=-1
#
# Graph Code 调用示例（server，语法糖：直接写复合节点名(...)；多数据出用“元组赋值”承接）：
# - 是否找到, 首次出现序号 = 整数列表_查找首次出现序号(输入列表=输入列表, 目标值=目标值)
...
class 整数列表_查找首次出现序号:
    ...
    def 查找(self, 输入列表: "整数列表", 目标值: "整数"):
        ...
        当前序号: "整数" = 0 + 0
        for 当前元素 in 待迭代列表:
            是否命中: "布尔值" = 是否相等(self.game, 输入1=当前元素, 输入2=目标值)
            if 是否命中:
