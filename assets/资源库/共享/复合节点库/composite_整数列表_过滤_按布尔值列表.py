"""
composite_id: composite_整数列表_过滤_按布尔值列表
node_name: 整数列表_按布尔掩码过滤
node_description: 根据布尔掩码（布尔值列表）过滤整数列表（等价于 zip + if 过滤）
scope: server
"""

# Python 等价写法：
# - 结果列表 = [值 for 值, 保留 in zip(输入列表, 保留条件列表) if 保留]
#
# 示例输入输出：
# - 输入列表=[10, 20, 30], 保留条件列表=[True, False, True] -> 结果列表=[10, 30]
#
# Graph Code 调用示例（server，语法糖：直接写复合节点名(...)）：
# - 结果列表: "整数列表" = 整数列表_按布尔掩码过滤(输入列表=输入列表, 保留条件列表=保留条件列表)
...
class 整数列表_按布尔掩码过滤:
    ...
    @flow_entry()
    def 过滤(self, 输入列表: "整数列表", 保留条件列表: "布尔值列表"):
        """返回过滤后的整数列表；长度按 zip 规则裁剪为两列表较短者。"""
        流程入("流程入")
