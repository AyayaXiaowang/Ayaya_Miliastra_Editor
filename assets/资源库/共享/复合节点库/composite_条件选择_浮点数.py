"""
composite_id: composite_条件选择_浮点数
node_name: 三元表达式_浮点数
node_description: 三元表达式（浮点数）：X if 条件 else Y
scope: server
"""
...
#
# 示例输入输出：
# - 条件=True,  条件为真输出=1.25, 条件为假输出=0.5 -> 选择结果=1.25
# - 条件=False, 条件为真输出=1.25, 条件为假输出=0.5 -> 选择结果=0.5
#
# Graph Code 示例（推荐写法：语法糖，自动改写为共享复合节点）：
# - 选择结果: "浮点数" = 条件为真输出 if 条件 else 条件为假输出
#
# Graph Code 调用示例（server，手动实例化）：
# - self._三元浮点 = 三元表达式_浮点数(self.game, self.owner_entity)
# - 选择结果: "浮点数" = self._三元浮点.按条件选择(条件=条件, 条件为真输出=条件为真输出, 条件为假输出=条件为假输出)
...
class 三元表达式_浮点数:
    ...
    - 该复合节点提供等价的“按条件选择浮点数”的单节点替代方案，便于在节点编辑器中复用。
    """
    ...
    @flow_entry()
    def 按条件选择(self, 条件: "布尔值", 条件为真输出: "浮点数", 条件为假输出: "浮点数"):
        """根据条件选择输出（浮点数）"""
        流程入("流程入")
        数据入("条件", pin_type="布尔值")
        数据入("条件为真输出", pin_type="浮点数")
        数据入("条件为假输出", pin_type="浮点数")
        数据出("选择结果", pin_type="浮点数", variable="选择结果")
        流程出("完成")
