"""
composite_id: composite_布尔值列表_任意为真
node_name: 布尔值列表_任意为真
node_description: 判断布尔值列表中是否存在任意一个为真（替代 any(列表)）
scope: server
"""
...
#
# 示例输入输出：
# - 输入列表=[False, True] -> 结果=True
# - 输入列表=[False, False] -> 结果=False
#
# Graph Code 示例（推荐写法：语法糖，自动改写为共享复合节点）：
# - 结果: "布尔值" = any(输入列表)
#
# Graph Code 调用示例（server，手动实例化）：
# - self._布尔任意为真 = 布尔值列表_任意为真(self.game, self.owner_entity)
# - 结果: "布尔值" = self._布尔任意为真.计算(输入列表=输入列表)
...
class 布尔值列表_任意为真:
    ...
    def 计算(self, 输入列表: "布尔值列表"):
        """判断输入列表中是否存在任意一个为真；空列表返回 False。"""
        数据入("输入列表", pin_type="布尔值列表")
        数据出("结果", pin_type="布尔值", variable="结果")

        结果: "布尔值" = 列表是否包含该值(self.game, 列表=输入列表, 值=True)
        return 结果
