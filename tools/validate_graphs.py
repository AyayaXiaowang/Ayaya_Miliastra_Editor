...
用法示例：
  ...
  - 常用单文件：
      python -X utf8 tools/validate_graphs.py -f assets/资源库/节点图/server/某图.py
  - 目录或通配符：
      python -X utf8 tools/validate_graphs.py assets/资源库/节点图/server
      python -X utf8 tools/validate_graphs.py "assets/资源库/节点图/**/*.py"
  - 行为开关：实体入参严格模式（仅允许连线/事件参数）
      python -X utf8 tools/validate_graphs.py --all --strict
"""
...
from engine.validate import EngineIssue, validate_files
...
def _collect_all_targets(workspace: Path) -> List[Path]:
    ...
    if graphs_dir.exists():
        files.extend(sorted(graphs_dir.rglob("*.py")))
    if composites_dir.exists():
        files.extend(sorted(composites_dir.rglob("*.py")))
    return files
...
def _expand_target_to_files(target_text: str, workspace: Path) -> List[Path]:
    ...
    if absolute_path.is_dir():
        return sorted(absolute_path.rglob("*.py"))
