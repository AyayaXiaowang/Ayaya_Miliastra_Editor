...
- 节点图内部错误请使用统一入口 `tools/validate_graphs.py` 或编辑器内的节点图校验功能。
...
# 统一工作空间根目录（脚本位于 tools/ 下），并保持本目录在 sys.path 前列以优先使用仓库内的 engine/app 实现
...
def _build_resource_context(
    ...
    """构造资源管理器与当前工作区下的全部存档视图。"""
    resource_manager = ResourceManager(workspace)
    package_index_manager = PackageIndexManager(workspace, resource_manager)
...
def _validate_packages(
    ...
    if not package_views:
        print_colored(
            "未找到任何存档包索引（assets/资源库/地图索引/pkg_*.json），跳过存档包校验。",
            Colors.YELLOW,
        )
        return 0, 0
...
def main() -> None:
    ...
    resource_manager, _, package_views = _build_resource_context(workspace_path)
