...
- **注册表 typed dict 默认值一致性**：当注册表声明使用 typed dict alias（例如 `字符串-整数字典`），会在注册表静态加载阶段校验 `default_value` 的键/值类型与值域一致性（例如整数字典禁止出现 `#RRGGBB` 颜色字符串），让 `validate-project` 能提前 fail-fast。
...
- **索引路径自愈**：当 graph_id 命中索引但物理文件不存在（常见：手动移动/重命名 `.py`）时，`ResourceManager.load_resource/load_graph_metadata/resource_exists` 会触发一次 `rebuild_index()` 并重试，以对齐磁盘现状。
...
- **目录骨架策略**：资源库初始化仅确保 `assets/资源库/{共享,项目存档}` 与共享根的各资源类型目录存在；不再在初始化阶段预创建“默认未归属项目存档”目录，避免只读场景/测试环境产生意外落盘。未归属落点仅在保存资源时按需创建。
