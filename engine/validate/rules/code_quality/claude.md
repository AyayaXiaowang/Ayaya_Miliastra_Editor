...
- 模块概览：`basic.py`（长连线/事件多出口/不可达）、`graph_errors.py`（IR/结构错误提升）、`unused_query_output.py`（未使用查询输出）、`entity_destroy_event_mount.py`（销毁事件挂载冲突）、`pull_eval_reevaluation_hazard.py`（拉取式重复求值风险）、`pull_eval_same_source_arith_hazard.py`（同源算术风险）、`dict_hazards.py`（字典引用语义风险）、`graph_model_utils.py`（共享解析/遍历工具）。
...
- `unused_query_output.py` 会将 `self.<复合实例>.<flow_entry>(...)` 视为有效“流程消费点”：通过解析宿主图 `__init__` 的 `self.<attr> = <复合类>(...)` 绑定，并从复合源码中提取 `@flow_entry/@event_handler` 方法名，避免复合节点成员方法调用导致的误报。
